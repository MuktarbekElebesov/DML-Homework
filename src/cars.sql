...
select sum(price) from cars where brand like 'Mercedes-Benz';

-- #13 Отобразите самую дорогую  и самую дешевую машину
(select max(price) from cars) union all (select min(price) from cars);
...
select * from cars where color =(select color from cars order by color desc limit 1);